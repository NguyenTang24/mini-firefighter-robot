...
static const char INDEX_HTML[] =
    ...
    ".reset{background:#7f8c8d}.pump{background:#8e44ad}"
    ...
    "<button class='pump'   onclick=\"post('/cmd_pump_test')\">PUMP TEST</button>"
    "<div id='status'>Loading...</div>"
    "<script>"
    "function post(url){fetch(url,{method:'POST'}).then(()=>refresh());}"
    "function refresh(){"
    "  fetch('/status').then(r=>r.json()).then(d=>{"
    "    document.getElementById('status').innerHTML='State: <b>'+d.state+'</b>';}"
    "  );}"
...
static esp_err_t handle_cmd_calib(httpd_req_t *req)
{
    xEventGroupSetBits(eg_commands, CMD_CALIB_BIT);
    httpd_resp_send(req, "OK", 2);
    ESP_LOGI(TAG, "CMD_CALIB");
    return ESP_OK;
}

static void pump_test_task(void *arg)
{
...
static esp_err_t handle_cmd_pump_test(httpd_req_t *req)
{
    xTaskCreate(pump_test_task, "pump_test", 1024, NULL, 3, NULL);
    httpd_resp_send(req, "OK", 2);
    ESP_LOGI(TAG, "CMD_PUMP_TEST");
    return ESP_OK;
}
...
static esp_err_t handle_status(httpd_req_t *req)
{
    ...
    snprintf(buf, sizeof(buf),
             "{\"state\":\"%s\",\"lidar_cm\":%d,\"flame\":%s,\"turn_ms\":%d,\"trim\":%d}",
             state_name(g_state), g_lidar_cm,
             g_flame_detected ? "true" : "false",
             g_turn_ms, g_motor_r_trim);
...
static void start_webserver(void)
{
    ...
    static const httpd_uri_t uris[] = {
        ...
        { .uri = "/cmd_calib",      .method = HTTP_POST, .handler = handle_cmd_calib      },
        { .uri = "/cmd_pump_test",  .method = HTTP_POST, .handler = handle_cmd_pump_test  },
