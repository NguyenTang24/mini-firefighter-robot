...
#define LOOP_MS           80
#define CONFIRM_FRAMES     1       // frames in a row before reporting a landmark
#define NUM_LANDMARKS      4       // 0=Red(off) 1=Blue 2=Green 3=Purple
                      ...
                      int dom_ch, int dom_margin)
{
    ...
    // real cards are dense enough, scattered noise is not
    bool dense_ok = (count * 100 >= box_area * 30);
...
#define CALIB_MODE 0   // set to 1 to print center-region RGB stats for calibration
...
static void camera_task(void *arg)
{
    ...
    while (1) {
...
#if CALIB_MODE
        {
            int cx = fb->width/2, cy = fb->height/2, half = 20;
            int rmin=31,gmin=63,bmin=31, rmax=0,gmax=0,bmax=0, n=0;
            long rsum=0, gsum=0, bsum=0;
            for (int y = cy-half; y < cy+half; y++) {
                for (int x = cx-half; x < cx+half; x++) {
                    int idx = (y * fb->width + x) * 2;
                    uint16_t p = ((uint16_t)fb->buf[idx]<<8)|fb->buf[idx+1];
            ...
            ESP_LOGI("CAL","R:%d-%d(%ld) G:%d-%d(%ld) B:%d-%d(%ld)",
        ...
        // LM:1 Orange/Red combined - single marker used for both turn and fire approach
        detections[1] = scan_blob(fb->buf, fb->width, fb->height,
                                  ORN_R_MIN, ORN_R_MAX,
                                  ORN_G_MIN, ORN_G_MAX,
                                  ORN_B_MIN, ORN_B_MAX, 0, 0);

        // LM:2 Green - dom_ch=2 (G dominant) filters false positives at distance
        detections[2] = scan_blob(fb->buf, fb->width, fb->height,
                                  GRN_R_MIN, GRN_R_MAX,
                                  GRN_G_MIN, GRN_G_MAX,
                                  GRN_B_MIN, GRN_B_MAX, 2, 6);

        // LM:3 disabled - orange and red merged into LM:1
        detections[3].found = false;
